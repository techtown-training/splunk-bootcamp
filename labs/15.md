...
## Instructions
As a final lab, you'll now configure a typical scenario when using Splunk. You'll install and configure an NGINX, then the Splunk forwarder to collect logs in Splunk. The idea is that you can apply everything you've learned with this lab.
...
Here's where the fun begins, you'll now need to hammer the NGINX server to generate enough traffic and then generate log entries to Splunk. You can use any load testing tool like [Apache Benchmark](https://httpd.apache.org/docs/2.4/programs/ab.html), [JMeter](https://jmeter.apache.org/), or the easiest one is to use [BlazeMeter](https://www.blazemeter.com/). Use different paths, make sure you include URLs that don't exist.
